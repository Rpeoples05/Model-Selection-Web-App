...
def logistic_model(X_train, y_train, hyperparameters):
    model = LogisticRegression(C=hyperparameters["C"], penalty='l2', max_iter=hyperparameters["max_iter"])
    model.fit(X_train, y_train)
    return model

def forest_model(X_train, y_train, hyperparameters):
    model = RandomForestClassifier(n_estimators=hyperparameters["n_estimators"],max_depth = hyperparameters["max_depth"], bootstrap = hyperparameters["bootstrap"], n_jobs = -1)
    model.fit(X_train, y_train)
    return model
